...
import org.example.kursinis.model.City;
import org.example.kursinis.model.Manager;
import org.example.kursinis.model.Product;
import org.example.kursinis.model.Warehouse;
    ...
    public void createWarehouse() {
        if (!update) {
            if (!addressTextField.getText().trim().isEmpty() && !cityComboBox.getSelectionModel().isEmpty()) {
                shopHibernate.createWarehouseInDB(productAddList.getItems(), managerAddList.getItems(), addressTextField.getText(), cityComboBox.getValue(), dateField.getValue());
