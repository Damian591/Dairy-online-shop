package org.example.kursinis.hibernate;
...
public class ShopHibernate extends GenericHibernate {
    ...
    public void createWarehouseInDB(List<Product> products, List<Manager> managers, String address, City city, LocalDate dateCreated) {
        //All must happen within the same transaction
        try {
            entityManager = getEntityManager();
            entityManager.getTransaction().begin();
            ...
            //Then we go through the objects that user wants to buy
            Warehouse warehouse = new Warehouse(address,new ArrayList<>(),new ArrayList<>(),city, dateCreated);
